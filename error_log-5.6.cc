...
***
*** valid for MySQL 5.5.x and MySQL 5.6.x
**/
...
static int error_log_fill_table(THD *thd, TABLE_LIST *tables, Item *cond)
{
  ...
  if (schema_table_store_record(thd, table))
    return 1;
  table->field[0]->store(log_error_file_ptr, strlen(log_error_file_ptr), system_charset_info);
  if (schema_table_store_record(thd, table))
    return 1;
 
  fd = fopen(log_error_file_ptr, "r"); 
  if (fd == NULL)
  {
